...
def predict_strength(inputs):
    # Reshape inputs to match the model's expected format
    input_array = np.array(inputs).reshape(1, -1)
    return model.predict(input_array)[0]

# Streamlit app UI
st.title("Geopolymer Concrete Strength Calculator")
st.write("Enter the values below to calculate the compressive strength (f'c) in MPa.")

# User inputs
SiO2 = st.number_input('SiO2 (%)', min_value=0.0, max_value=100.0, step=0.1)
Al2O3 = st.number_input('Al2O3 (%)', min_value=0.0, max_value=100.0, step=0.1)
Fe2O3 = st.number_input('Fe2O3 (%)', min_value=0.0, max_value=100.0, step=0.1)
CaO = st.number_input('CaO (%)', min_value=0.0, max_value=100.0, step=0.1)
Waste = st.number_input('Waste (kg/m³)', min_value=0.0, max_value=1000.0, step=1.0)

Na2O = st.number_input('Na2O (%)', min_value=0.0, max_value=100.0, step=0.1)
SiO2_1 = st.number_input('SiO2.1 (%)', min_value=0.0, max_value=100.0, step=0.1)
Water = st.number_input('Water (%)', min_value=0.0, max_value=100.0, step=0.1)
NaOH = st.number_input('NaOH (%)', min_value=0.0, max_value=100.0, step=0.1)
SS_SH = st.number_input('SS/SH (ratio)', min_value=0.0, max_value=10.0, step=0.1)

Curing_Temperature = st.number_input('Curing Temperature (°C)', min_value=0.0, max_value=200.0, step=1.0)
Curing_Time = st.number_input('Curing Time (hours)', min_value=0.0, max_value=48.0, step=0.1)
Coarse = st.number_input('Coarse Aggregate (kg/m³)', min_value=0.0, max_value=2000.0, step=1.0)
Fine = st.number_input('Fine Aggregate (kg/m³)', min_value=0.0, max_value=2000.0, step=1.0)

# Button to calculate
if st.button('Predict Compressive Strength'):
    inputs = [SiO2, Al2O3, Fe2O3, CaO, Waste, Na2O, SiO2_1, Water, NaOH, SS_SH, Curing_Temperature, Curing_Time, Coarse, Fine]
    result = predict_strength(inputs)
    st.success(f"The predicted compressive strength (f'c) is: {result:.2f} MPa")
...
streamlit run geo_calculator.py